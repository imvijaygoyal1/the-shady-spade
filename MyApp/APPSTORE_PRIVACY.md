...
The script also deploys a small Worker shim so `/join/{CODE}`, `/scorekeeper/{CODE}`, and `/scorecard/{CODE}` render fallback pages while `.well-known/apple-app-site-association` is served as `application/json`.
...
curl -L https://shadyspade.vijaygoyal.org/scorecard/FINAL1 | tee /tmp/shadyspade-scorecard.html | rg "Final Scorecard" && rg "FINAL1" /tmp/shadyspade-scorecard.html
...
- Published final scorecards must remain explicit, user-started, read-only for viewers, and separate from temporary live scorekeeper sessions and leaderboard upload consent.
